...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 44889 or testnet: 44890)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22888 or testnet: 44888)") + "\n" +
        ...
        "\n" + _("SSL options: (see the FedoraCoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FedoraCoin is probably already running."), strDataDir.c_str()));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                pusers = new CUserDB(nCoinDBCache, false, fReindex);
