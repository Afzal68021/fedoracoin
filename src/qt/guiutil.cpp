...
{
    if(uri.scheme() != QString("fedoracoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("fedoracoin://"))
    {
        uri.replace(0, 11, "fedoracoin:");
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a fedoracoin.desktop file to the autostart directory:
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FedoraCoin-Qt") + " " + tr("version") + " " +
    ...
    setWindowTitle(tr("FedoraCoin-Qt"));
